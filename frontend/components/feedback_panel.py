...
def render_feedback_form(message_id: str, user_query: str, thread_id: str, agent_type: str):
    ...
    with st.expander("🔍 提交详细反馈", expanded=True):
        ...
        description = st.text_area(
            ...
            key=f"feedback_desc_{message_id}"
        )
        ...
        if feedback_type == "建议合并实体":
            ...
            with col1:
                merge_from = st.text_input(
                    "要合并的实体",
                    placeholder="例如: 学生管理办法",
                    key=f"merge_from_{message_id}"
                )
            with col2:
                merge_to = st.text_input(
                    "合并到",
                    placeholder="例如: 学生管理规定",
                    key=f"merge_to_{message_id}"
                )
            ...
            with col1:
                source = st.text_input(
                    "源实体",
                    placeholder="例如: 学生",
                    key=f"rel_source_{message_id}"
                )
                old_type = st.text_input(
                    "错误的关系类型",
                    placeholder="例如: 申请",
                    key=f"rel_old_type_{message_id}"
                )
            with col2:
                target = st.text_input(
                    "目标实体",
                    placeholder="例如: 奖学金",
                    key=f"rel_target_{message_id}"
                )
                new_type = st.text_input(
                    "正确的关系类型",
                    placeholder="例如: 获得",
                    key=f"rel_new_type_{message_id}"
                )
            ...
            missing_entity = st.text_input(
                ...
                key=f"missing_entity_{message_id}"
            )

            entity_type = st.text_input(
                "实体类型",
                placeholder="例如: 奖学金类型",
                key=f"missing_entity_type_{message_id}"
            )
            ...
            hallucination_entity = st.text_input(
                "幻觉实体ID或名称",
                placeholder="例如: 虚构的奖学金",
                key=f"hallucination_entity_{message_id}"
            )

            content = {
                "entity_id": hallucination_entity
            }
...
def render_entity_feedback_menu(entity_id: str, entity_name: str):
    ...
    feedback_type = st.radio(
        ...
        key=f"entity_feedback_type_{entity_id}"
    )

    description = st.text_area(
        "说明",
        key=f"entity_feedback_desc_{entity_id}"
    )
    ...
    if feedback_type == "建议合并实体":
        merge_to = st.text_input(
            "合并到哪个实体？",
            key=f"entity_merge_to_{entity_id}"
        )
