...
def display_entity_management():
    ...
    with operation_tabs[0]:
        ...
        with col2:
            ...
            selected_type = st.selectbox("实体类型", options=["全部"] + entity_types, key="entity_search_type")
    ...
    with operation_tabs[1]:
        ...
        with st.form("create_entity_form"):
            ...
            entity_type = (
                st.selectbox("实体类型 *", options=entity_types)
                if entity_types
                else st.text_input("实体类型 *", placeholder="输入实体类型")
            )
        ...
        if entity_id_to_update:
            ...
            if lookup_button or "entity_to_update" in st.session_state:
                try:
                    # 如果没有缓存或ID发生变化，重新查询
                    if (
                        "entity_to_update" not in st.session_state
                        or st.session_state.entity_to_update is None
                        or st.session_state.entity_to_update.get("id") != entity_id_to_update
                    ):
                    ...
                    if "entity_to_update" in st.session_state and st.session_state.entity_to_update is not None:
                        ...
                        with st.form("update_entity_form"):
                            ...
                            try:
                                ...
                                type_index = (
                                    entity_types_list.index(current_type) if current_type in entity_types_list else 0
                                )
                            except Exception as e:
                    ...
                    try:
                        with st.spinner("正在删除实体..."):
                            ...
                            if result is not None and result.get("success", False):
                                ...
                                # 如果缓存中有此实体，清除缓存
                                if (
                                    "entity_to_update" in st.session_state
                                    and st.session_state.entity_to_update is not None
                                    and st.session_state.entity_to_update.get("id") == delete_id
                                ):
                                    del st.session_state.entity_to_update
                            else:
                                error_message = "未知错误"
...
def display_relation_management():
    ...
    with operation_tabs[0]:
        ...
        with col3:
            ...
            selected_rel_type = st.selectbox("关系类型", options=["全部"] + relation_types, key="relation_search_type")
    ...
    with operation_tabs[1]:
        ...
        with st.form("create_relation_form"):
            ...
            relation_type = (
                st.selectbox("关系类型 *", options=relation_types)
                if relation_types
                else st.text_input("关系类型 *", placeholder="输入关系类型")
            )
    ...
    with operation_tabs[2]:
        ...
        with col2:
            ...
            relation_type_filter = st.selectbox(
                "关系类型", options=["全部"] + relation_types_list, key="update_relation_type"
            )
                ...
                with st.spinner("正在查找关系..."):
                    ...
                    if relations is not None and len(relations) > 0:
                        ...
                        selected_relation = st.selectbox(
                            "选择要更新的关系", options=relation_ids, key="relation_to_update_id"
                        )
                        ...
                        if selected_relation:
                            ...
                            if relation is not None:
                                ...
                                with st.form("update_relation_form"):
                                    ...
                                    # 关系类型
                                    new_type = st.selectbox("关系类型", options=rel_types, index=type_index)
                                    ...
                                    new_weight = st.slider(
                                        "关系权重", min_value=0.0, max_value=1.0, value=current_weight, step=0.01
                                    )
    ...
    with operation_tabs[3]:
        ...
        with col2:
            ...
            delete_type = (
                st.selectbox("关系类型 *", options=relation_types, key="delete_relation_type")
                if relation_types
                else st.text_input("关系类型 *", key="delete_relation_type_input")
            )
                ...
                if st.button("删除关系", key="delete_relation_button"):
                    try:
                        with st.spinner("正在删除关系..."):
                            # 构建删除数据
                            delete_data = {"source": delete_source, "type": delete_type, "target": delete_target}
