...
def display_performance_stats():
    ...
    # 计算消息响应时间统计
    message_times = [m["duration"] for m in st.session_state.performance_metrics if m["operation"] == "send_message"]
    ...
    # 反馈性能统计
    feedback_times = [m["duration"] for m in st.session_state.performance_metrics if m["operation"] == "send_feedback"]
        ...
        def wrapper(*args, **kwargs):
            ...
            if "performance_metrics" in st.session_state:
                st.session_state.performance_metrics.append(
                    {"operation": endpoint or func.__name__, "duration": duration, "timestamp": time.time()}
                )
...
def display_enhanced_performance_stats():
    ...
    if "performance_metrics" in st.session_state and st.session_state.performance_metrics:
        message_times = [
            m["duration"] for m in st.session_state.performance_metrics if m["operation"] == "send_message"
        ]
...
def decorate_api_functions():
    """为API函数添加性能监控装饰器"""
    try:
        from frontend.utils.api import get_knowledge_graph, get_source_content, send_feedback, send_message
