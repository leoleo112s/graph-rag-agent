...
def send_message_stream(message: str, on_token: Callable[[str, bool], None]) -> str:
    ...
    try:
        ...
        # 非阻塞模式发起请求
        response = requests.post(
            f"{API_URL}/chat/stream", json=params, stream=True, headers={"Accept": "text/event-stream"}
        )
...
def send_feedback(message_id: str, query: str, is_positive: bool, thread_id: str, agent_type: str = "graph_agent"):
    ...
    try:
        ...
        st.session_state.performance_metrics.append(
            {"operation": "send_feedback", "duration": duration, "timestamp": time.time(), "is_positive": is_positive}
        )
...
def get_knowledge_graph(limit: int = 100, query: str = None) -> Dict:
    ...
    try:
        ...
        response = requests.get(f"{API_URL}/knowledge_graph", params=params, timeout=30)
...
def get_knowledge_graph_from_message(message: str, query: str = None):
    ...
    try:
        ...
        response = requests.get(f"{API_URL}/knowledge_graph_from_message", params=params, timeout=30)
        result = response.json()
...
def get_source_content(source_id: str) -> Dict:
    ...
    try:
        response = requests.post(f"{API_URL}/source", json={"source_id": source_id}, timeout=30)
        result = response.json()
...
def get_source_file_info(source_id: str) -> dict:
    ...
    try:
        response = requests.post(f"{API_URL}/source_info", json={"source_id": source_id}, timeout=10)
        result = response.json()
...
def get_source_file_info_batch(source_ids: list) -> dict:
    ...
    """
    try:
        response = requests.post(f"{API_URL}/source_info_batch", json={"source_ids": source_ids}, timeout=10)
        return response.json()
    except requests.exceptions.RequestException as e:
...
def get_kg_reasoning(reasoning_type, entity_a, entity_b=None, max_depth=3, algorithm=community_algorithm):
    ...
    try:
        ...
        # 使用JSON格式发送请求
        response = requests.post(f"{API_URL}/kg_reasoning", json=params, timeout=60)  # 社区检测可能需要更长时间
...
def get_entity_types():
    """获取所有实体类型"""
    try:
        response = requests.get(f"{API_URL}/entity_types", timeout=10)
        result = response.json()
...
def get_relation_types():
    """获取所有关系类型"""
    try:
        response = requests.get(f"{API_URL}/relation_types", timeout=10)
        result = response.json()
...
def get_entities(filters=None):
    ...
    try:
        ...
        response = requests.post(f"{API_URL}/entities/search", json=filters, timeout=20)
        result = response.json()
...
def get_relations(filters=None):
    ...
    try:
        ...
        response = requests.post(f"{API_URL}/relations/search", json=filters, timeout=20)
        result = response.json()
...
def create_entity(entity_data):
    """创建新实体"""
    try:
        response = requests.post(f"{API_URL}/entity/create", json=entity_data, timeout=15)
        return response.json()
    except requests.exceptions.RequestException as e:
...
def update_entity(entity_data):
    """更新实体"""
    try:
        response = requests.post(f"{API_URL}/entity/update", json=entity_data, timeout=15)
        return response.json()
    except requests.exceptions.RequestException as e:
...
def delete_entity(entity_id):
    """删除实体"""
    try:
        response = requests.post(f"{API_URL}/entity/delete", json={"id": entity_id}, timeout=15)
        return response.json()
    except requests.exceptions.RequestException as e:
...
def create_relation(relation_data):
    """创建新关系"""
    try:
        response = requests.post(f"{API_URL}/relation/create", json=relation_data, timeout=15)
        return response.json()
    except requests.exceptions.RequestException as e:
...
def update_relation(relation_data):
    """更新关系"""
    try:
        response = requests.post(f"{API_URL}/relation/update", json=relation_data, timeout=15)
        return response.json()
    except requests.exceptions.RequestException as e:
...
def delete_relation(relation_data):
    """删除关系"""
    try:
        response = requests.post(f"{API_URL}/relation/delete", json=relation_data, timeout=15)
        return response.json()
    except requests.exceptions.RequestException as e:
...
def clear_chat():
    ...
    try:
        ...
        # 清除后端状态
        response = requests.post(f"{API_URL}/clear", json={"session_id": st.session_state.session_id})
...
class ApiBatchProcessor:
    ...
    def add_request(self, request_type, request_data, callback):
        ...
        if request_type not in self.queues:
            ...
            # 启动处理线程
            self.threads[request_type] = threading.Thread(target=self._process_queue, args=(request_type,), daemon=True)
    ...
    def _batch_get_content(self, chunk_ids):
        """批量获取内容"""
        try:
            response = requests.post(f"{API_URL}/content_batch", json={"chunk_ids": chunk_ids}, timeout=30)
            return response.json()
        except requests.exceptions.RequestException as e:
            print(f"批量获取内容错误: {e}")
            return {}
