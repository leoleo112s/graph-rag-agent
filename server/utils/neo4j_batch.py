    ...
    def get_source_info_batch(source_ids: List[str], driver) -> Dict[str, Dict]:
        ...
        try:
            ...
            if chunk_ids:
                ...
                chunk_results = driver.execute_query(chunk_query, {"ids": chunk_ids}, result_transformer_=Result.to_df)
            ...
            if community_ids:
                ...
                community_results = driver.execute_query(
                    community_query, {"ids": community_ids}, result_transformer_=Result.to_df
                )
        ...
        try:
            ...
            if direct_chunk_ids:
                ...
                chunk_results = driver.execute_query(
                    chunk_query, {"ids": direct_chunk_ids}, result_transformer_=Result.to_df
                )
                ...
                if not chunk_results.empty:
                    for _, row in chunk_results.iterrows():
                        ...
                        for original_id in chunk_ids:
                            if chunk_id == original_id or (
                                len(original_id.split(",")) >= 2 and original_id.split(",")[-1] == chunk_id
                            ):
