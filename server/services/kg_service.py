...
def get_chunks(limit: int = 10, offset: int = 0):
    ...
    try:
        ...
        result = driver.execute_query(
            query, parameters={"limit": int(limit), "offset": int(offset)}, result_transformer_=Result.to_df
        )
    ...
    try:
        ...
        for record in result.records:
            ...
            if neighbor:
                ...
                # 添加到A和B的连接
                links.append({"source": entity_a, "target": neighbor_id, "label": "连接", "weight": 1})

                links.append({"source": neighbor_id, "target": entity_b, "label": "连接", "weight": 1})
...
def get_all_paths(driver, entity_a, entity_b, max_depth=3):
    ...
    try:
        ...
        check_result = driver.execute_query(check_query, {"entity_a": entity_a, "entity_b": entity_b})

        if not check_result.records or len(check_result.records) == 0:
            return {"error": f"实体 '{entity_a}' 或 '{entity_b}' 不存在", "nodes": [], "links": []}
        ...
        for record in result.records:
            ...
            if path:
                ...
                if cycle_rels:
                    ...
                    cycle_info = {"description": cycle_str, "length": cycle_length}
...
def get_simplified_community(driver, entity_id, max_depth=2):
    ...
    try:
        ...
        check_result = driver.execute_query(check_query, {"entity_id": entity_id})

        # 如果实体不存在，返回错误信息
        if not check_result.records or len(check_result.records) == 0:
            print(f"实体不存在: {entity_id}")
            return {"error": f"实体 '{entity_id}' 不存在", "nodes": [], "links": []}
        ...
        # 执行查询并处理结果
        neighbors_result = driver.execute_query(neighbors_query, {"entity_id": entity_id})
        ...
        relations_result = driver.execute_query(relations_query, {"entity_ids": entity_ids})
        ...
        for record in relations_result.records:
            ...
            if source_id and target_id and rel_type:
                ...
                if link_key not in link_map:
                    link_data = {"source": source_id, "target": target_id, "label": rel_type, "weight": 1}
                    links.append(link_data)
                    link_map[link_key] = link_data
