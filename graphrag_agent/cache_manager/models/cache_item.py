...
class CacheItem:
    ...
    def is_high_quality(self) -> bool:
        """判断是否为高质量缓存"""
        return (
            self.metadata.get("user_verified", False)
            or self.metadata.get("quality_score", 0) > 2
            or self.metadata.get("fast_path_eligible", False)
        )
    ...
    def to_json(self, ensure_ascii: bool = False) -> str:
        ...
        try:
            ...
            # 如果序列化失败，返回错误信息
            return json.dumps({"content": f"Serialization failed: {str(e)}", "metadata": self.metadata})
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "CacheItem":
        ...
        try:
            ...
            # 返回错误缓存项，确保程序不会崩溃
            return cls(
                f"Error deserializing cache item: {str(e)}",
                {
                    "created_at": time.time(),
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"CacheItem(content='{content_preview}', "
