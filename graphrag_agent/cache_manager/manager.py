...
from .backends import CacheStorageBackend, HybridCacheBackend, MemoryCacheBackend, ThreadSafeCacheBackend
...
class CacheManager:
    ...
    def __init__(
        ...
        enable_vector_similarity = (
            cache_config["enable_vector_similarity"] if enable_vector_similarity is None else enable_vector_similarity
        )
        similarity_threshold = (
            cache_config["similarity_threshold"] if similarity_threshold is None else similarity_threshold
        )
        max_vectors = cache_config["max_vectors"] if max_vectors is None else max_vectors

        # 设置存储后端
        backend = self._create_storage_backend(storage_backend, memory_only, cache_dir, max_memory_size, max_disk_size)
        ...
        self.performance_metrics = {"exact_hits": 0, "vector_hits": 0, "misses": 0, "total_queries": 0}

    def _create_storage_backend(
        self, storage_backend, memory_only, cache_dir, max_memory_size, max_disk_size
    ) -> CacheStorageBackend:
        """创建存储后端"""
        if storage_backend:
            ...
            return HybridCacheBackend(cache_dir=cache_dir, memory_max_size=max_memory_size, disk_max_size=max_disk_size)
