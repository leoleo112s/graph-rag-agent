...
class DiskCacheBackend(CacheStorageBackend):
    ...
    def __init__(
        self, cache_dir: str = "./cache", max_size: int = 1000, batch_size: int = 10, flush_interval: float = 30.0
    ):
        """初始化磁盘缓存后端"""
        ...
        with self._lock:
            ...
            if key in self.metadata:
                ...
                self.metadata[key].update(
                    {"last_accessed": current_time, "access_count": self.metadata[key].get("access_count", 0)}
                )
                self.metadata.move_to_end(key)
            else:
                # 新增项
                self.metadata[key] = {"created_at": current_time, "last_accessed": current_time, "access_count": 0}

            # 添加到写入队列
            self.write_queue.append((key, value))

            # 检查是否需要刷新
            if len(self.write_queue) >= self.batch_size or (time.time() - self.last_flush_time) > self.flush_interval:
