...
class GraphProjectionMixin:
    ...
    def create_projection(self) -> Tuple[Any, Dict]:
        ...
        try:
            ...
            print(f"图投影创建成功: {result.get('nodeCount', 0)} 节点, " f"{result.get('relationshipCount', 0)} 关系")
    ...
    def _create_filtered_projection(self, total_node_count: int) -> Tuple[Any, Dict]:
        ...
        try:
            ...
            # 创建过滤投影
            config = {
                "nodeProjection": {"__Entity__": {"properties": ["*"], "filter": f"id(node) IN {important_nodes}"}},
            ...
            print(f"过滤投影创建成功: {result.get('nodeCount', 0)} 节点, " f"{result.get('relationshipCount', 0)} 关系")
            return self.G, result
    ...
    def _create_conservative_projection(self) -> Tuple[Any, Dict]:
        ...
        try:
            # 使用最小配置
            config = {"nodeProjection": "__Entity__", "relationshipProjection": "*"}
    ...
    def _create_minimal_projection(self) -> Tuple[Any, Dict]:
        ...
        try:
            ...
            # 创建最小化投影
            minimal_config = {
                "nodeProjection": {"__Entity__": {"filter": f"id(node) IN {critical_nodes}"}},
                "relationshipProjection": "*",
            }

            self.G, result = self.gds.graph.project(self.projection_name, minimal_config)
