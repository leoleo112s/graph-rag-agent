...
class SLLPADetector(GraphProjectionMixin, BaseCommunityDetector):
    ...
    def detect_communities(self) -> Dict[str, Any]:
        ...
        try:
            # 执行SLLPA算法
            result = self.gds.sllpa.write(self.G, writeProperty="communityIds", **self._get_optimized_sllpa_params())

            community_count = result.get("communityCount", 0)
            iterations = result.get("iterations", 0)

            print(f"SLLPA算法完成: {community_count} 个社区, " f"{iterations} 次迭代")

            return {"communityCount": community_count, "iterations": iterations}
    ...
    def _get_optimized_sllpa_params(self) -> Dict[str, Any]:
        ...
        if self.memory_mb > 32 * 1024:  # >32GB
            return {"maxIterations": 100, "minAssociationStrength": 0.05, "concurrency": GDS_CONCURRENCY}
        elif self.memory_mb > 16 * 1024:  # >16GB
            return {"maxIterations": 80, "minAssociationStrength": 0.08, "concurrency": max(1, GDS_CONCURRENCY - 1)}
        else:  # 小内存
            return {"maxIterations": 50, "minAssociationStrength": 0.1, "concurrency": max(1, GDS_CONCURRENCY // 2)}
    ...
    def _save_communities_fallback(self) -> Dict[str, int]:
        ...
        try:
            ...
            return {"saved_communities": count, "note": "使用了简化保存方法"}
        except Exception as e:
