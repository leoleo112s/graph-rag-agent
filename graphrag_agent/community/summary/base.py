    ...
    def prepare_string(data: Dict) -> str:
        ...
        try:
            ...
            for node in data.get("nodes", []):
                ...
                node_description = (
                    f", description: {node['description']}" if "description" in node and node["description"] else ""
                )
            ...
            for rel in data.get("rels", []):
                ...
                description = (
                    f", description: {rel['description']}" if "description" in rel and rel["description"] else ""
                )
...
class BaseCommunityRanker:
    ...
    def calculate_ranks(self) -> None:
        ...
        try:
            ...
            print(f"社区权重计算完成，处理了 {processed_count} 个社区，" f"耗时: {time.time() - start_time:.2f}秒")
        except Exception as e:
            print(f"计算社区权重时出错: {e}")
    ...
    def store_summaries(self, summaries: List[Dict]) -> None:
        ...
        for i in range(0, len(summaries), batch_size):
            ...
            try:
                ...
                print(f"已存储批次 {i//batch_size + 1}/{total_batches}, " f"耗时: {time.time() - batch_start:.2f}秒")
...
class BaseSummarizer(ABC):
    ...
    def process_communities(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"开始并行生成 {len(community_info)} 个社区摘要，" f"使用 {optimal_workers} 个线程...")

            summaries = self._process_communities_parallel(community_info, optimal_workers)
            ...
            self._print_performance_stats(total_time, rank_time, self.query_time, self.llm_time, self.store_time)
    ...
    def _process_communities_parallel(self, community_info: List[Dict], workers: int) -> List[Dict]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            future_to_community = {
                executor.submit(self._process_single_community, info): i for i, info in enumerate(community_info)
            }
            ...
            for i, future in enumerate(concurrent.futures.as_completed(future_to_community)):
                try:
                    ...
                    if (i + 1) % 10 == 0 or (i + 1) == len(community_info):
                        print(f"已处理 {i+1}/{len(community_info)} " f"({(i+1)/len(community_info)*100:.1f}%)")
    ...
    def _process_single_community(self, community: Dict) -> Dict:
        ...
        try:
            ...
            return {"community": community_id, "summary": summary, "full_content": stringify_info}
        except Exception as e:
            print(f"处理社区 {community_id} 摘要时出错: {e}")
            return {"community": community_id, "summary": f"生成摘要时出错: {str(e)}", "full_content": str(community)}

    def _print_performance_stats(
        self, total_time: float, rank_time: float, query_time: float, llm_time: float, store_time: float
    ) -> None:
        """打印性能统计信息"""
