...
class RetrievalPrecision(BaseMetric):
    ...
    def calculate_metric(self, data) -> Tuple[Dict[str, float], List[float]]:
        ...
        self.log(
            f"总体评分分布: 最低={min(precision_scores):.4f}, 最高={max(precision_scores):.4f}, 平均={avg_precision:.4f}"
        )
        self.log("完成检索精确率评估")
...
class RetrievalUtilization(BaseMetric):
    ...
    def calculate_metric(self, data) -> Tuple[Dict[str, float], List[float]]:
        ...
        self.log(
            f"总体评分分布: 最低={min(utilization_scores):.4f}, 最高={max(utilization_scores):.4f}, 平均={avg_utilization:.4f}"
        )
        self.log("完成检索利用率评估")
                    ...
                    if result.records and len(result.records) > 0:
                        ...
                        if chunk_text:
                            ...
                            if key_phrases:
                                # 计算关键短语在回答中出现的比例
                                matched_phrases = sum(
                                    1 for phrase in key_phrases if phrase.lower() in answer_text.lower()
                                )
