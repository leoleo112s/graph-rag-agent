    ...
    def calculate_metric(self, data) -> Tuple[Dict[str, float], List[float]]:
        ...
        for idx, sample in enumerate(data.samples):
            ...
            if processed_relationships:
                self.log(
                    f"  关系示例: {processed_relationships[:2]}{'...' if len(processed_relationships) > 2 else ''}"
                )
...
class GraphCoverageMetric(BaseMetric):
    ...
    def _evaluate_graph_coverage(self, sample, keywords: List[str]) -> float:
        ...
        total_score = base_score + 0.7 * (0.3 * structure_score + 0.4 * relevance_score + 0.3 * connectedness_score)
    ...
    def _calculate_structure_score(
        self, entity_count: int, rel_count: int, entity_info: Dict[str, str], relationship_info: List[Dict]
    ) -> float:
    ...
    def _calculate_relevance_score(
        self, keywords: List[str], entity_info: Dict[str, str], relationship_info: List[Dict]
    ) -> float:
        """计算相关性得分 - 基于关键词匹配度"""
        if not keywords:
...
class RelationshipUtilizationMetric(BaseMetric):
    ...
    def calculate_metric(self, data) -> Tuple[Dict[str, float], List[float]]:
        ...
        for idx, sample in enumerate(data.samples):
            ...
            total_score = base_score + 0.7 * (0.3 * quantity_score + 0.4 * quality_score + 0.3 * relevance_score)
    ...
    def _calculate_quality_score(self, rel_info: List[Dict[str, Any]]) -> float:
        ...
        if weighted_rels:
            return 0.3 * description_ratio + 0.3 * type_diversity + 0.2 * validity_ratio + 0.2 * weight_score
        else:
            # 如果没有权重信息，重新分配占比
            return 0.4 * description_ratio + 0.3 * type_diversity + 0.3 * validity_ratio
