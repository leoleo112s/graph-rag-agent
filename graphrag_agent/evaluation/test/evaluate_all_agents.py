...
def parse_args():
    ...
    parser.add_argument("--save_dir", type=str, default="./evaluation_results/comparison", help="评估结果保存目录")
    parser.add_argument("--questions_file", type=str, required=True, help="要评估的问题文件（JSON格式）")
    parser.add_argument("--golden_answers_file", type=str, default=None, help="标准答案文件（JSON格式，可选）")
    parser.add_argument("--verbose", action="store_true", help="是否打印详细评估过程")
    parser.add_argument(
        "--metrics", type=str, default="", help="要评估的指标，用逗号分隔，留空则使用所有Agent共有的指标"
    )
    parser.add_argument(
        "--agents",
        type=str,
        default="graph,hybrid,naive,fusion,deep",
    ...
    parser.add_argument("--skip_missing", action="store_true", help="如果某个Agent无法加载，跳过它而不是终止评估")
    parser.add_argument("--use_deeper", action="store_true", help="是否为Deep Research Agent使用增强版工具")
...
def main():
    ...
    # 加载问题和答案
    try:
        questions, golden_answers = load_questions_and_answers(args.questions_file, args.golden_answers_file)
    ...
    # 创建评估器
    config = {"save_dir": args.save_dir, "debug": args.verbose, "metrics": metrics, "neo4j_client": neo4j, "llm": llm}
