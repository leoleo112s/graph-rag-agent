...
def extract_references_from_answer(answer: str) -> Dict[str, Any]:
    ...
    """
    # 初始化结果
    result = {"entities": [], "relationships": [], "chunks": [], "reports": []}
...
def extract_entities_from_text(text: str) -> List[str]:
    """直接从文本中提取实体ID"""
    # 尝试匹配实体ID部分
    entity_matches = re.search(r"[Ee]ntities\s*[=:]\s*\[(.*?)\]", text, re.DOTALL) or re.search(
        r"[Ee]ntities\s*[=:]\s*([\d\s,]+)", text, re.DOTALL
    )
...
def extract_relationships_from_text(text: str) -> List[str]:
    ...
    # 尝试匹配关系ID部分
    rel_matches = (
        re.search(r"[Rr]elationships\s*[=:]\s*\[(.*?)\]", text, re.DOTALL)
        or re.search(r"[Rr]elationships\s*[=:]\s*([\d\s,]+)", text, re.DOTALL)
        or re.search(r"[Rr]eports\s*[=:]\s*\[(.*?)\]", text, re.DOTALL)
        or re.search(r"[Rr]eports\s*[=:]\s*([\d\s,]+)", text, re.DOTALL)
    )
...
def extract_reports_from_text(text: str) -> List[str]:
    """直接从文本中提取报告ID"""
    # 尝试匹配报告ID部分
    report_matches = re.search(r"[Rr]eports\s*[=:]\s*\[(.*?)\]", text, re.DOTALL) or re.search(
        r"[Rr]eports\s*[=:]\s*([\d\s,]+)", text, re.DOTALL
    )
