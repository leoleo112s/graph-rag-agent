...
class CompositeGraphRAGEvaluator:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # 默认的答案质量评估指标
        default_answer_metrics = ["em", "f1", "response_coherence", "answer_comprehensiveness", "factual_consistency"]
        ...
        passed_metrics = [
            m
            for m in self.config.get_metrics()
    ...
    def evaluate_with_golden_answers(
        self, agent_name: str, questions: List[str], golden_answers: List[str]
    ) -> Dict[str, float]:
        """
        使用标准答案评估特定Agent
        ...
        for i, (question, golden_answer) in enumerate(zip(questions, golden_answers)):
            ...
            # 创建答案评估样本
            answer_sample = AnswerEvaluationSample(question=question, golden_answer=golden_answer)

            # 创建检索评估样本
            retrieval_sample = RetrievalEvaluationSample(question=question)
            ...
            try:
                ...
                # 立即将答案添加到列表，避免后续重复调用
                answers.append({"question": question, "answer": clean_answer})
                ...
                answers.append({"question": question, "answer": error_message})
    ...
    def compare_agents_with_golden_answers(
        self, questions: List[str], golden_answers: List[str]
        ...
        for agent_name, agent in self.agents.items():
            ...
            for i, question in enumerate(questions):
                ...
                if cache_key in processed_questions:
                    answers.append({"question": question, "answer": processed_questions[cache_key]})
                    self.log(f"    使用缓存的回答")
                    continue
                ...
                try:
                    ...
                    answers.append({"question": question, "answer": f"获取回答时出错: {str(e)}"})
