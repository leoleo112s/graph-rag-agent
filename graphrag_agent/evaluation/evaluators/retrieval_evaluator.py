...
class GraphRAGRetrievalEvaluator(BaseEvaluator):
    ...
    def evaluate(self, data: RetrievalEvaluationData) -> Dict[str, float]:
        ...
        for i, sample in enumerate(data.samples):
            ...
            self.log(
                f"    关系: {refs.get('relationships', [])[:5]}{'...' if len(refs.get('relationships', [])) > 5 else ''}"
            )
        ...
        if self.entity_map:
            ...
            for ent_id in sample.referenced_entities:
                if ent_id in self.entity_map:
                    ...
                    enhanced_entity = {"id": ent_id, "description": desc}
                    enhanced_entities.append(enhanced_entity)
                else:
                    enhanced_entities.append({"id": ent_id, "description": f"实体 {ent_id}"})
    ...
    def _enhance_relation_data(self, sample):
        ...
        for rel_id in string_rel_ids:
            if rel_id in self.relation_map:
                ...
                enhanced_relation = (rel_data["source"], rel_data["relation"], rel_data["target"])
    ...
    def evaluate_agent(self, agent_name: str, questions: List[str]) -> Dict[str, float]:
        ...
        for question in questions:
            # 创建评估样本
            sample = RetrievalEvaluationSample(question=question, agent_type=agent_name)
        ...
        try:
            ...
            if len(entities) < 3:
                ...
                if chunk_ids:
                    ...
                    chunk_entity_result = self.neo4j_client.execute_query(chunk_entity_query, {"chunk_ids": chunk_ids})
