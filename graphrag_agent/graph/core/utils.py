...
def ensure_vector_index(
    ...
    def _drop_index_if_needed(
        ...
        mismatch_similarity = (
            existing_similarity and isinstance(existing_similarity, str) and existing_similarity.lower() != similarity
        )

        if mismatch_dimension or mismatch_similarity or not existing_config:
            print(f"重新创建向量索引 {index_name} 以应用" f" dim={dim} similarity={similarity} 配置")
...
def batch_process(
    items: List[Any], process_func: Callable, batch_size: int = 100, show_progress: bool = True
) -> List[Any]:
    """
    批量处理项目
...
def get_performance_stats(total_time: float, time_records: Dict[str, float]) -> Dict[str, str]:
    """
    生成性能统计摘要
