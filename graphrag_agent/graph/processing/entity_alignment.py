...
from graphrag_agent.config.settings import ALIGNMENT_CONFLICT_THRESHOLD, ALIGNMENT_MIN_GROUP_SIZE
...
class EntityAligner:
    ...
    def __init__(self):
        ...
        # 性能统计
        self.stats = {"groups_found": 0, "conflicts_detected": 0, "entities_aligned": 0}
    ...
    def group_by_canonical_id(self, skip: int = 0, limit: int = 100) -> Dict[str, List[str]]:
        ...
        results = self.graph.query(query, params={"min_size": ALIGNMENT_MIN_GROUP_SIZE, "skip": skip, "limit": limit})
    ...
    def count_alignment_groups(self) -> int:
        ...
        result = self.graph.query(query, params={"min_size": ALIGNMENT_MIN_GROUP_SIZE})
    ...
    def detect_conflicts(self, canonical_id: str, entity_ids: List[str]) -> Dict[str, Any]:
        ...
        if all_rel_types:
            ...
            return {"has_conflict": has_conflict, "jaccard_similarity": jaccard, "entities": entities}
    ...
    def resolve_conflict(self, canonical_id: str, conflict_info: Dict[str, Any]) -> str:
        ...
        # 构建LLM提示
        entity_desc = "\n".join(
            [f"- {e['entity_id']}: {e['description']}, {e['rel_count']} relations" for e in entities]
        )
    ...
    def merge_entities(self, canonical_id: str, entity_ids: List[str], keep_id: Optional[str] = None) -> int:
        ...
        try:
            result = self.graph.query(merge_query, params={"target_id": target_id, "to_delete": to_delete})
