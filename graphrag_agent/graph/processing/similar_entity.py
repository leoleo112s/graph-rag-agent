...
class SimilarEntityDetector:
    ...
    def __init__(self, config: GDSConfig = None):
        ...
        self.gds = GraphDataScience(self.config.uri, auth=(self.config.username, self.config.password))
    ...
    def detect_similar_entities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"备用KNN执行完成，写入 {fallback_result['relationshipsWritten']} 个关系, 用时: {self.knn_time:.2f}秒"
                )
    ...
    def detect_communities(self) -> Dict[str, Any]:
        ...
        try:
            # 使用WCC算法
            result = self.gds.wcc.write(self.G, writeProperty="wcc", relationshipTypes=["SIMILAR"], consecutiveIds=True)
