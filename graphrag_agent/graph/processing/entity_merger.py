...
from graphrag_agent.config.settings import ENTITY_BATCH_SIZE
from graphrag_agent.config.settings import MAX_WORKERS as DEFAULT_MAX_WORKERS
from graphrag_agent.graph.core import connection_manager, get_performance_stats, print_performance_stats, timer
...
class EntityMerger:
        ...
        for batch_start in range(0, candidate_count, optimal_batch_size):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                future_to_candidates = {
                    executor.submit(self._process_candidate_group, candidates): i for i, candidates in enumerate(batch)
                }
    ...
    def _process_candidate_group(self, candidates: List[str]) -> Optional[str]:
        ...
        for retry in range(max_retries + 1):
            try:
                # 调用LLM进行分析
                answer = self.chain.invoke({"chat_history": chat_history, "entities": candidates})
        ...
        for batch_index in range(total_batches):
            ...
            try:
                ...
                if result:
                    ...
                    print(
                        f"已处理合并批次 {batch_index+1}/{total_batches}, "
    ...
    def process_duplicates(self, duplicate_candidates: List[Any]) -> Tuple[int, Dict[str, Any]]:
        ...
        # 过滤处理数量过少的候选组
        filtered_candidates = [candidates for candidates in fixed_candidates if len(candidates) > 1]
        ...
        print(f"生成合并建议完成，用时 {suggestion_elapsed:.2f} 秒, " f"找到 {len(merge_groups)} 组可合并实体")
        ...
        # 返回性能统计摘要
        time_records = {"LLM处理时间": self.llm_time, "解析时间": self.parse_time, "数据库时间": self.db_time}
