...
from graphrag_agent.config.settings import EMBEDDING_BATCH_SIZE
...
class EmbeddingManager:
    ...
    def __init__(self, batch_size: int = 100, max_workers: int = 4):
        ...
        # 处理统计
        self.stats = {"entity_updates": 0, "chunk_updates": 0, "total_updates": 0, "errors": 0}
    ...
    def update_entity_embeddings(self, entity_ids: Optional[List[str]] = None, limit: int = 1000) -> int:
            ...
            try:
                ...
                for j, entity_id in enumerate(entity_ids):
                    if j < len(embeddings) and embeddings[j] is not None:
                        updates.append({"neo4j_id": neo4j_ids[j], "embedding": embeddings[j]})
                ...
                self.console.print(
                    f"[green]批次 {i//self.batch_size + 1} 更新完成，"
                    f"处理了 {len(batch)} 个实体，"
        ...
        self.console.print(
            f"[blue]实体Embedding更新完成，共更新 {updated_count} 个实体，"
    ...
    def update_chunk_embeddings(self, chunk_ids: Optional[List[str]] = None, limit: int = 1000) -> int:
        ...
        self.console.print(
            f"[blue]Chunk Embedding更新完成，共更新 {updated_count} 个Chunk，"
            f"耗时: {time.time() - start_time:.2f}秒[/blue]"
        )
    ...
    def display_stats(self):
        ...
        self.console.print(
            f"[blue]总耗时: {self.total_time:.2f}秒，"
