...
class EntityIndexManager(BaseIndexer):
    ...
    def create_vector_index(self, node_label: str = "__Entity__", embedding_property: str = "embedding") -> None:
        ...
        try:
            ...
            logger.info(f"✅ Vector index created/verified: {ENTITY_VECTOR_INDEX}")
        except Exception as e:
            # 索引可能已存在，这不是错误
            if "already exists" in str(e).lower() or "equivalent" in str(e).lower():
                logger.info(f"✅ Vector index already exists: {ENTITY_VECTOR_INDEX}")
            else:
    ...
    def create_entity_index(
        ...
        if not entities:
            logger.info("没有找到需要处理的实体节点（可能已存在 embeddings）")
        ...
        logger.info(f"开始为 {len(entities)} 个实体生成 embeddings")
