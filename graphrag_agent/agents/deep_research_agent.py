...
class DeepResearchAgent(BaseAgent):
    ...
    def _generate_node(self, state):
        ...
        if global_result:
            self._log_execution("generate", {"question": question, "source": "全局缓存"}, "全局缓存命中")
        ...
        if cached_result:
            self._log_execution("generate", {"question": question, "source": "会话缓存"}, "会话缓存命中")
            # 将命中内容同步到全局缓存
        ...
        try:
            ...
            # 生成回答
            response = chain.invoke({"thinking": thinking, "question": question, "response_type": response_type})
    ...
    def explore_knowledge(self, query: str, thread_id: str = "default"):
        ...
        # 准备探索参数
        explore_query = {"query": query, "entities": entities}
        ...
        if isinstance(result, dict) and "exploration_path" in result:
            ...
            if "session_id" in result:
                self.session_context[thread_id] = {"exploration_session_id": result["session_id"]}
            ...
            enhanced_result = {
                ...
                "discovered_entities": [
                    step.get("node_id") for step in result.get("exploration_path", []) if step.get("step", 0) > 0
                ],
    ...
    def is_deeper_tool(self, use_deeper=True):
        ...
        if use_deeper:
            ...
            try:
                ...
                self.use_deeper_tool = False
                self.stream_tool = (
                    self.research_tool.get_thinking_stream_tool()
                    if hasattr(self.research_tool, "get_thinking_stream_tool")
                    else None
                )
                return f"切换到增强版失败: {e}"
        else:
