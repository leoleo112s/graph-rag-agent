...
class GraphAgent(BaseAgent):
    ...
    def _add_retrieval_edges(self, workflow):
        ...
        # 添加条件边，根据文档评分决定路由
        workflow.add_conditional_edges("retrieve", self._grade_documents, {"generate": "generate", "reduce": "reduce"})
    ...
    def _generate_node(self, state):
        ...
        if global_result:
            self._log_execution("generate", {"question": question, "docs_length": len(docs)}, "全局缓存命中")
        ...
        if cached_result:
            self._log_execution("generate", {"question": question, "docs_length": len(docs)}, "会话缓存命中")
            # 将命中内容同步到全局缓存
    ...
    def _reduce_node(self, state):
        ...
        if cached_result:
            self._log_execution("reduce", {"question": question, "docs_length": len(docs)}, cached_result)
            return {"messages": [AIMessage(content=cached_result)]}
        ...
        self._log_execution("reduce", {"question": question, "docs_length": len(docs)}, response)
                ...
                try:
                    ...
                    if local_text and len(local_text) > 100:
                        ...
                        workflow_state["messages"][-1] = ToolMessage(
                            content=local_text, tool_call_id="local_search", name="local_search_tool"
                        )
    ...
    async def _retrieve_node_async(self, state):
        ...
        try:
            ...
            if not tool_calls:
                print("无法获取工具调用信息")
                return {"messages": [AIMessage(content="无法获取查询信息，请重试。")]}
            ...
            # 返回正确格式的工具消息
            return {"messages": [ToolMessage(content=tool_text, tool_call_id=tool_id, name=tool_name)]}
        except Exception as e:
            # 处理错误
            import traceback
            ...
            return {"messages": [AIMessage(content=f"搜索过程中出现错误: {str(e)}")]}
