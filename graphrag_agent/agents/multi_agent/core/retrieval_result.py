...
class RetrievalMetadata(BaseModel):
    ...
    # 数据源类型
    source_type: Literal["document", "chunk", "entity", "relationship", "community", "subgraph"] = Field(
        description="数据源类型"
    )

    # 置信度 (0.0-1.0)
    confidence: float = Field(default=0.5, ge=0.0, le=1.0, description="检索结果的置信度")

    # 数据时间戳（用于过滤过期数据）
    timestamp: datetime = Field(default_factory=datetime.now, description="数据的时间戳")

    # DO层级（Digital Object层级）
    do_level: Optional[Literal["L0-DO", "L1-DO", "L2-DO"]] = Field(
        default=None, description="Digital Object层级，仅DO粒度时有效"
    )

    # 社区ID（LocalSearch时有效）
    community_id: Optional[str] = Field(default=None, description="所属社区ID")

    # 图遍历跳数（GraphSearch时有效）
    hop_count: Optional[int] = Field(default=None, ge=0, description="图遍历的跳数")

    # 额外元数据（扩展用）
    extra: Dict[str, Any] = Field(default_factory=dict, description="额外的自定义元数据")
...
class RetrievalResult(BaseModel):
    ...
    # 结果唯一标识
    result_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="检索结果唯一标识符")
    ...
    # 检索内容（可以是文本或结构化数据）
    evidence: Union[str, Dict[str, Any]] = Field(description="检索到的内容，可以是文本或结构化数据")
    ...
    # 相似度/相关性分数 (0.0-1.0)
    score: float = Field(default=0.5, ge=0.0, le=1.0, description="相似度或相关性分数")
