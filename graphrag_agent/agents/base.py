...
from graphrag_agent.cache_manager.manager import CacheManager, ContextAwareCacheKeyStrategy, HybridCacheBackend
...
from graphrag_agent.models.get_models import get_embeddings_model, get_llm_model, get_stream_llm_model
...
class BaseAgent(ABC):
    ...
    def __init__(self, cache_dir="./cache", memory_only=False):
        ...
        self.cache_manager = CacheManager(
            ...
            storage_backend=(
                HybridCacheBackend(cache_dir=cache_dir, memory_max_size=200, disk_max_size=2000)
                if not memory_only
                else None
            ),
            cache_dir=cache_dir,
        ...
        self.global_cache_manager = CacheManager(
            ...
            storage_backend=(
                HybridCacheBackend(cache_dir=f"{cache_dir}/global", memory_max_size=500, disk_max_size=5000)
                if not memory_only
                else None
            ),
    ...
    async def _stream_process(self, inputs: Dict[str, Any], config: Dict[str, Any]) -> AsyncGenerator[str, None]:
        ...
        # 构建状态字典
        state = {"messages": messages, "configurable": config.get("configurable", {})}
    ...
    def _log_execution(self, node_name: str, input_data: Any, output_data: Any):
        ...
        self.execution_log.append(
            {"node": node_name, "timestamp": time.time(), "input": input_data, "output": output_data}
        )
    ...
    def check_fast_cache(self, query: str, thread_id: str = "default") -> str:
        ...
        self._log_performance("fast_cache_check", {"duration": duration, "hit": result is not None})
    ...
    def _check_all_caches(self, query: str, thread_id: str = "default"):
        ...
        if global_result:
            ...
            self._log_performance("cache_check", {"duration": cache_time, "type": "global"})
        ...
        if fast_result:
            ...
            cache_time = time.time() - cache_check_start
            self._log_performance("cache_check", {"duration": cache_time, "type": "fast"})
        ...
        if cached_response:
            ...
            cache_time = time.time() - cache_check_start
            self._log_performance("cache_check", {"duration": cache_time, "type": "standard"})

            return cached_response

        # 没有命中任何缓存
        cache_time = time.time() - cache_check_start
        self._log_performance("cache_check", {"duration": cache_time, "type": "miss"})
    ...
    def ask_with_trace(self, query: str, thread_id: str = "default", recursion_limit: Optional[int] = None) -> Dict:
        ...
        recursion_limit = recursion_limit if recursion_limit is not None else self.default_recursion_limit
        ...
        try:
            ...
            self._log_performance(
                "ask_with_trace",
                {"total_duration": overall_time, "cache_check": cache_time, "processing": process_time},
            )
    ...
    async def _try_global_cache(self, query: str) -> AsyncGenerator[str, None]:
        """尝试从全局缓存获取并流式返回结果"""
        global_result = self.global_cache_manager.get(query)
        if global_result:
            normalized = self._normalize_response_to_string(global_result)
            async for chunk in self._stream_cached_response(normalized):
                yield chunk

    async def _try_fast_cache(self, query: str, thread_id: str) -> AsyncGenerator[str, None]:
        """尝试从快速缓存获取并流式返回结果"""
        fast_result = self.check_fast_cache(query, thread_id)
        if fast_result:
            normalized = self._normalize_response_to_string(fast_result)
            async for chunk in self._stream_cached_response(normalized):
                yield chunk
            # 将命中的内容同步到全局缓存
            self.global_cache_manager.set(query, normalized)

    async def _try_regular_cache(self, query: str, thread_id: str) -> AsyncGenerator[str, None]:
        """尝试从常规缓存获取并流式返回结果"""
        cached_response = self.cache_manager.get(query, thread_id=thread_id)
        if cached_response:
            normalized = self._normalize_response_to_string(cached_response)
            async for chunk in self._stream_cached_response(normalized):
                yield chunk
            # 将命中的内容同步到全局缓存
            self.global_cache_manager.set(query, normalized)
    ...
    async def ask_stream(
        ...
        # 全局缓存
        cached_chunks = []
        async for chunk in self._try_global_cache(safe_query):
        ...
        # 快速缓存
        async for chunk in self._try_fast_cache(safe_query, thread_id):
            cached_chunks.append(chunk)
            yield chunk
        if cached_chunks:
            return
        ...
        # 常规缓存
        async for chunk in self._try_regular_cache(safe_query, thread_id):
            cached_chunks.append(chunk)
            yield chunk
        if cached_chunks:
            return
        ...
        try:
            ...
            self._log_performance("ask_stream", {"total_duration": process_time, "processing": process_time})
    ...
    def mark_answer_quality(self, query: str, is_positive: bool, thread_id: str = "default"):
        ...
        self._log_performance("mark_quality", {"duration": mark_time, "is_positive": is_positive})
    ...
    def clear_cache_for_query(self, query: str, thread_id: str = "default"):
        ...
        try:
            ...
            if hasattr(self, "global_cache_manager") and hasattr(
    ...
    def _validate_answer(self, query: str, answer: str, thread_id: str = "default") -> bool:
        ...
        def validator(query, answer):
            ...
            # 检查是否包含错误消息
            error_patterns = ["抱歉，处理您的问题时遇到了错误", "技术原因:", "无法获取", "无法回答这个问题"]
