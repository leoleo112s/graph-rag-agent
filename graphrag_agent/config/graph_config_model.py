    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "schema": {
                    "entities": ["法条", "条款", "参数", "标准"],
                    "relations": ["规定", "约束", "定义", "包含"]
                },
                "bridge_mappings": [
                    {"bridge_key": "bridge_issue_type", "role": "适用场景"}
                ]
            }
        }
        ...
        return GraphConfig(
            ...
            domain_definitions=[
                DomainDefinition(
                    ...
                    bridge_mappings=[
                        BridgeMapping(bridge_key="bridge_issue_type", role="适用场景")
                    ]
                ),
                DomainDefinition(
                    domain_name="案件库",
                ...
                DomainDefinition(
                    ...
                    schema=DomainSchema(
                        entities=["处理方案", "调解方法", "建议", "经验"],
                        relations=["适用于", "解决", "参考", "借鉴"]
                    ),
                    bridge_mappings=[
                        BridgeMapping(bridge_key="bridge_issue_type", role="适用问题")
                    ]
                )
            ]
        )
        ...
        return GraphConfig(
            ...
            domain_definitions=[
                DomainDefinition(
                    ...
                    schema=DomainSchema(
                        entities=["产品", "参数", "规格", "功能", "配件"],
                        relations=["具有", "支持", "包含", "兼容"]
                    ),
                    bridge_mappings=[
                        BridgeMapping(bridge_key="bridge_product_sku", role="产品标识")
                    ]
                ),
                DomainDefinition(
                    domain_name="工单记录",
                ...
                DomainDefinition(
                    ...
                    schema=DomainSchema(
                        entities=["政策", "条款", "标准", "期限"],
                        relations=["规定", "适用", "要求", "限制"]
                    ),
                    bridge_mappings=[
                        BridgeMapping(bridge_key="bridge_issue_type", role="适用场景")
                    ]
        ...
        return GraphConfig(
            ...
            domain_definitions=[
                DomainDefinition(
                    ...
                    schema=DomainSchema(
                        entities=["疾病", "药物", "治疗方法", "诊断标准"],
                        relations=["治疗", "诊断", "引起", "预防"]
                    ),
                    bridge_mappings=[
                ...
                DomainDefinition(
                    ...
                    schema=DomainSchema(
                        entities=["患者", "病情", "检查", "诊断", "医生"],
                        relations=["患有", "检查", "诊断为", "开具"]
                    ),
                    bridge_mappings=[
                ...
                DomainDefinition(
                    ...
                    schema=DomainSchema(
                        entities=["药品", "用法", "剂量", "疗程"],
                        relations=["用于", "治疗", "配合", "禁忌"]
                    ),
                    bridge_mappings=[
                        BridgeMapping(bridge_key="bridge_disease", role="适应症")
                    ]
                )
            ]
        )
