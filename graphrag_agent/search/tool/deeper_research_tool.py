...
class DeeperResearchTool:
    ...
    def __init__(self, config=None, llm=None, embeddings=None, graph=None):
        ...
        # 初始化增强模块
        # 1. 社区感知搜索增强器
        self.community_search = CommunityAwareSearchEnhancer(self.graph, self.embeddings, self.llm)

        # 2. 动态知识图谱构建器
        self.knowledge_builder = DynamicKnowledgeGraphBuilder(
            self.graph,
            EntityRelationExtractor(
                self.llm, system_template_build_graph, human_template_build_graph, entity_types, relationship_types
            ),
        )

        # 3. Chain of Exploration检索器
        self.chain_explorer = ChainOfExplorationSearcher(self.graph, self.llm, self.embeddings)
    ...
    def thinking(self, query: str) -> Dict[str, Any]:
        ...
        if community_summaries:
            ...
            for i, summary in enumerate(community_summaries):
                ...
                self.evidence_tracker.add_evidence(
                    strategy_step_id, f"community_summary_{i}", summary, "community_knowledge"
                )
        ...
        if graph_entities:
            ...
            kg_step_id = self.evidence_tracker.add_reasoning_step(
                query_id, "knowledge_graph_analysis", f"构建了初始知识图谱，识别出核心实体: {central_entity_ids}"
            )
            ...
            self.evidence_tracker.add_evidence(kg_step_id, "knowledge_graph", json.dumps(kg_info), "graph_structure")
        ...
        for iteration in range(self.deep_research.max_iterations):
            ...
            iteration_step_id = self.evidence_tracker.add_reasoning_step(
                query_id, f"iteration_{iteration}", f"\n开始第 {iteration + 1} 轮迭代思考\n"
            )
            ...
            if iteration == 0 and initial_sub_queries:
                ...
                if result["status"] == "empty":
                    self._log("\n[深度研究] 生成的思考内容为空")
                    self.evidence_tracker.add_evidence(
                        iteration_step_id, "generation_result", "生成的思考内容为空", "reasoning_status"
                    )
                    ...
                    self._log("\n[深度研究] AI认为已有足够信息生成答案")
                    self.evidence_tracker.add_evidence(
                        iteration_step_id, "reasoning_complete", "AI认为已有足够信息生成答案", "reasoning_status"
                    )
                    break
            ...
            if not queries_to_process and self.deep_research.all_retrieved_info:
                ...
                if followup_queries:
                    ...
                    followup_step_id = self.evidence_tracker.add_reasoning_step(
                        query_id, "followup_queries_generation", f"基于已收集的信息生成跟进查询: {followup_queries}"
                    )
            ...
            if not queries_to_process:
                if not self.deep_research.all_retrieved_info and iteration == 0:
                    ...
                    self._log("\n[深度研究] 没有检索到信息，使用原始查询")
                    self.evidence_tracker.add_evidence(
                        iteration_step_id, "query_fallback", "没有检索到信息，使用原始查询", "reasoning_status"
                    )
                else:
                    # 已有信息，结束迭代
            ...
            for search_query in queries_to_process:
                ...
                search_step_id = self.evidence_tracker.add_reasoning_step(
                    query_id, search_query, f"执行查询: {search_query}"
                )
                ...
                if self.thinking_engine.has_executed_query(search_query):
                    ...
                    self.evidence_tracker.add_evidence(
                        search_step_id, "duplicate_query", "已搜索过该查询，跳过重复执行", "search_status"
                    )
                    continue
                ...
                if "chunks" in kbinfos:
                    for chunk in kbinfos.get("chunks", []):
                        ...
                        if chunk_id and chunk_text:
                            # 记录chunk作为证据
                            self.evidence_tracker.add_evidence(search_step_id, chunk_id, chunk_text, "document_chunk")

                            # 尝试从chunk中提取实体和关系
                            # 减少实体提取，只对部分内容执行
                            if (
                                hasattr(self.knowledge_builder, "extractor")
                                and self.knowledge_builder.extractor
                                and chunks_count < 5
                            ):  # 只有当chunks数量较少时才提取
                                try:
                                    ...
                                    if extract_cache_key not in self._extract_cache:
                                        self.knowledge_builder.extract_subgraph_from_chunk(chunk_text, chunk_id)
                ...
                # 检查搜索结果是否为空
                has_results = (
                    kbinfos.get("chunks", []) or kbinfos.get("entities", []) or kbinfos.get("relationships", [])
                )
                ...
                if not has_results:
                    ...
                    self.evidence_tracker.add_evidence(
                        search_step_id, "no_results", f"没有找到与查询相关的信息: {search_query}", "search_result"
                    )
                    continue
                ...
                if extract_cache_key in self._extraction_cache:
                    ...
                    extract_prompt = RELEVANT_EXTRACTION_PROMPT.format(
                        prev_reasoning=truncated_prev_reasoning, search_query=search_query, document=kb_prompt_result
                    )
                    ...
                    summary_think = (
                        extraction_msg.content if hasattr(extraction_msg, "content") else str(extraction_msg)
                    )
                    # 缓存提取结果
                ...
                # 保存重要信息
                has_useful_info = (
                    "**Final Information**" in summary_think and "No helpful information found" not in summary_think
                )
            ...
            if iteration > 0 and self.deep_research.all_retrieved_info:
                ...
                if gap_check_key in self._gap_check_cache:
                    ...
                    gap_needed = (
                        len(
                            self.query_generator.generate_followup_queries(query, self.deep_research.all_retrieved_info)
                        )
                        > 0
                    )
        ...
        # 记录最终回答步骤
        final_step_id = self.evidence_tracker.add_reasoning_step(query_id, "final_answer", "生成最终答案")

        self.evidence_tracker.add_evidence(
            final_step_id, "final_answer_evidence", retrieved_content, "synthesized_knowledge"
        )
    ...
    def get_stream_tool(self):
        ...
        class DeeperResearchStreamTool(BaseTool):
            ...
            description: str = (
                "增强版流式深度研究工具：通过社区感知和知识图谱分析，结合多轮推理和搜索解决复杂问题，支持流式输出。"
            )
    ...
    async def search_stream(self, query_input: Any) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            if keywords.get("high_level", []):
                ...
                # 我们会在后台处理结果
                asyncio.create_task(
                    self.chain_explorer.explore_async(query, keywords.get("high_level", [])[:3], max_steps=3)
                )
        ...
        for iteration in range(self.deep_research.max_iterations):
            ...
            for search_query in queries_to_process:
                ...
                if self.deep_research.thinking_engine.has_executed_query(search_query):
                    ...
                    self.evidence_tracker.add_evidence(
                        iteration_step_id, "duplicate_query", "已搜索过该查询，跳过重复执行", "search_status"
                    )
                    continue
                ...
                # 使用异步LLM提取有用信息
                summary_think = await self.deep_research._async_extract_info(
                    search_query, truncated_prev_reasoning, kb_prompt_result
                )
            ...
            try:
                ...
                if central_entities:
                    ...
                    for entity in central_entities:
                        ...
                        enhanced_prompt += (
                            f"- {entity_id} (重要性: {importance:.3f}, 类型: {entity_type}): {description}\n"
                        )
