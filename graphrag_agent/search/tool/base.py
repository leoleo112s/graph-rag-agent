...
class BaseSearchTool(ABC):
    ...
    def __init__(self, cache_dir: str = "./cache/search"):
        ...
        self.cache_manager = CacheManager(
            ...
            storage_backend=MemoryCacheBackend(max_size=BASE_SEARCH_CONFIG["cache_max_size"]),
            cache_dir=cache_dir,
        )
    ...
    def semantic_search(
        self, query: str, entities: List[Dict], embedding_field: str = "embedding", top_k: int = None
    ) -> List[Dict]:
        """
        对一组实体进行语义相似度搜索
        ...
        try:
            ...
            # 使用工具类进行排序
            return VectorUtils.rank_by_similarity(query_embedding, entities, embedding_field, top_k)
        except Exception as e:
            print(f"语义搜索失败: {e}")
    ...
    def filter_by_relevance(self, query: str, docs: List, top_k: int = None) -> List:
        ...
        try:
            ...
            return VectorUtils.filter_documents_by_relevance(query_embedding, docs, top_k=limit)
        except Exception as e:
            print(f"文档过滤失败: {e}")
            limit = top_k or self.default_relevance_top_k
