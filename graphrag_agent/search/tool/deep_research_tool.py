...
class DeepResearchTool(BaseSearchTool):
    ...
    def __init__(self):
        ...
        self.dual_searcher = DualPathSearcher(self._kb_retrieve, self._kg_retrieve, KB_NAME)
    ...
    def _create_kb_retrieval_func(self):
        ...
        def kb_retrieve(question: str, limit: int = 5):
            ...
            try:
                ...
                if not result:
                    print("\n[KB检索] 搜索结果为空")
                    return {"chunks": [], "doc_aggs": [], "entities": [], "relationships": [], "Chunks": []}
                ...
                if chunk_ids:
                    for chunk_id in chunk_ids[:limit]:
                        ...
                        if not any(d.get("doc_id") == doc_id for d in doc_aggs):
                            doc_aggs.append({"doc_id": doc_id, "title": f"Document: {doc_id}"})
                elif isinstance(result, dict) and not chunk_ids:
                    answer_text = result.get("answer") or result.get("final_answer") or result.get("response")
                    if answer_text and len(str(answer_text)) > 10:
                        ...
                        doc_aggs.append({"doc_id": "text", "title": "Document: text"})
        ...
        try:
            ...
            response = self.llm.invoke(
                FINAL_ANSWER_PROMPT.format(
                    query=query, retrieved_content=retrieved_content, thinking_process=thinking_process
                )
            )
    ...
    async def _async_extract_info(self, search_query, prev_reasoning, kb_prompt_result):
        ...
        extract_prompt = RELEVANT_EXTRACTION_PROMPT.format(
            prev_reasoning=prev_reasoning, search_query=search_query, document=kb_prompt_result
        )
    ...
    def thinking(self, query: str):
        ...
        for iteration in range(self.max_iterations):
            ...
            if not queries_to_process and self.all_retrieved_info:
                followup_queries = self.query_generator.generate_followup_queries(query, self.all_retrieved_info)
