...
class DynamicKnowledgeGraphBuilder:
    ...
    def build_query_graph(self, query: str, entities: List[str], depth: int = 2) -> nx.DiGraph:
        ...
        print(
            f"构建查询图谱完成，包含 {self.knowledge_graph.number_of_nodes()} 个实体和 "
    ...
    def _explore_graph(self, entities: List[str], current_depth: int, max_depth: int):
        ...
        try:
            ...
            # 执行查询
            relationships = self.graph.query(query, params={"entity_ids": entities})
            ...
            for rel in relationships:
                ...
                if target not in self.knowledge_graph:
                    self.knowledge_graph.add_node(
                        target, type="entity", properties={"description": rel.get("target_description", "")}
                    )
                    new_entities.append(target)

                # 添加边
                if not self.knowledge_graph.has_edge(source, target):
                    self.knowledge_graph.add_edge(source, target, type=relation)

            # 递归探索新发现的实体
            if new_entities:
                self._explore_graph(new_entities, current_depth + 1, max_depth)
    ...
    def build_hierarchical_graph(self, documents):
        ...
        for doc in documents:
            ...
            # 添加文档节点
            self.knowledge_graph.add_node(
                doc_id, type="document", properties={"title": doc.get("title", ""), "type": doc.get("type", "")}
            )
            ...
            for section in doc.get("sections", []):
                ...
                for i, paragraph in enumerate(section.get("paragraphs", [])):
                    ...
                    self.knowledge_graph.add_node(
                        para_id, type="paragraph", properties={"content": paragraph, "index": i}
                    )
                    # 添加章节到段落的关系
        ...
        print(
            f"构建层级图谱完成，包含 {self.knowledge_graph.number_of_nodes()} 个节点和 "
        ...
        try:
            ...
            for match in entity_pattern.findall(extraction_result):
                ...
                if entity_id not in self.knowledge_graph:
                    self.knowledge_graph.add_node(
                        entity_id,
                        type=entity_type,
                        properties={"description": description, "source": f"chunk:{chunk_id}"},
                    )
            ...
            for match in relationship_pattern.findall(extraction_result):
                ...
                for node_id in [source_id, target_id]:
                    if node_id not in self.knowledge_graph:
                        self.knowledge_graph.add_node(
                            node_id,
                            type="unknown",
                            properties={"description": "从关系中提取的实体", "source": f"chunk:{chunk_id}"},
                        )
                ...
                self.knowledge_graph.add_edge(
                    ...
                    target_id,
                    type=rel_type,
                    properties={"description": description, "weight": float(weight), "source": f"chunk:{chunk_id}"},
                )
    ...
    def get_central_entities(self, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            # 排序
            top_entities = sorted(pagerank.items(), key=lambda x: x[1], reverse=True)[:limit]
            ...
            # 合并入度和出度
            total_degree = {
                node: in_degree.get(node, 0) + out_degree.get(node, 0) for node in set(in_degree) | set(out_degree)
            }

            # 排序
            top_entities = sorted(total_degree.items(), key=lambda x: x[1], reverse=True)[:limit]
