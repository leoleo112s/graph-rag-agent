...
class DualPathSearcher:
    ...
    def _merge_results(self, result1: Dict, result2: Dict) -> Dict:
        ...
        """
        # 初始化结果字典
        result = {"chunks": result1.get("chunks", []).copy(), "doc_aggs": result1.get("doc_aggs", []).copy()}
    ...
    def generate_multiple_hypotheses(query: str, llm) -> List[str]:
        ...
        try:
            ...
            potential_hypotheses = [
                line
                for line in lines
                if len(line) > 10 and not line.startswith("假设") and not line.startswith("以下是")
            ]
    ...
    def generate_followup_queries(self, original_query: str, retrieved_info: List[str]) -> List[str]:
        ...
        try:
            ...
            response = self.llm.invoke(
                self.followup_query_prompt.format(original_query=original_query, retrieved_info=info_text)
            )
            content = response.content if hasattr(response, "content") else str(response)
