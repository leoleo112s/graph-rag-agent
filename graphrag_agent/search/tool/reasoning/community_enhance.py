...
class CommunityAwareSearchEnhancer:
    ...
    def find_relevant_communities(self, query: str, keywords: Dict[str, List[str]], top_k: int = 3) -> List[Dict]:
        """
        查找与查询最相关的社区
        ...
        try:
            ...
            for comm in communities:
                ...
                try:
                    ...
                    similarity = cosine_similarity(
                        np.array(query_embedding).reshape(1, -1), np.array(comm_embedding).reshape(1, -1)
                    )[0][0]
                    ...
                    kw_score = sum(1 for kw in high_level_kw if kw.lower() in comm["summary"].lower()) * 2.0
                    kw_score += sum(0.5 for kw in low_level_kw if kw.lower() in comm["summary"].lower())
                    ...
                    final_score = similarity * 0.6 + (min(kw_score, 5) / 5.0) * 0.3 + importance_norm * 0.1

                    scored_communities.append(
                        {"community_id": comm["community_id"], "score": final_score, "summary": comm["summary"]}
                    )
            ...
            # 排序并返回top_k个社区
            return sorted(scored_communities, key=lambda x: x["score"], reverse=True)[:top_k]
    ...
    def extract_community_knowledge(self, communities: List[Dict]) -> Dict:
        ...
        try:
            ...
            for summary in summaries:
                ...
                summaries_with_time.append({"summary": summary, "temporal_info": time_info})

            # 返回增强的结构化知识
            return {"entities": entities, "relationships": relationships, "summaries": summaries_with_time}
    ...
    def generate_search_strategy(self, query: str, community_knowledge: Dict) -> Dict:
        """
        基于社区知识生成搜索策略
        ...
        # 如果没有足够的社区信息，返回基本策略
        if len(entities) < 3:
            return {"strategy_type": "basic", "follow_up_queries": [], "focus_entities": []}
