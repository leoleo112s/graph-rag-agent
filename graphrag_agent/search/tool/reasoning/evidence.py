...
class EvidenceChainTracker:
    ...
    def add_reasoning_step(self, query_id: str, search_query: str, reasoning: str) -> str:
        """
        添加推理步骤
    ...
    def add_evidence_with_confidence(
        self, step_id: str, source_id: str, content: str, source_type: str, confidence=0.5, metadata=None
    ):
        """
        添加带可信度得分的证据
                ...
                for num1_info in numbers1:
                    for num2_info in numbers2:
                        ...
                        if self._context_similarity(num1_info["context"], num2_info["context"]) > 0.7:
                            # 检查数值是否不同
                            if abs(num1_info["value"] - num2_info["value"]) > 0.001 * max(
                                num1_info["value"], num2_info["value"]
                            ):
        ...
        if hasattr(self, "llm") and self.llm:
            for i in range(len(evidences)):
                for j in range(i + 1, len(evidences)):
                    # 检查是否已经发现数值矛盾
                    if any(
                        c["evidence1"] == evidence_ids[i] and c["evidence2"] == evidence_ids[j] for c in contradictions
                    ):
                        continue
                    ...
                    # 使用LLM检测矛盾
                    contradiction = self._detect_semantic_contradiction(
                        content1, content2, evidence_ids[i], evidence_ids[j]
                    )
                    if contradiction:
    ...
    def _extract_numbers_with_context(self, text):
        ...
        for match in matches:
            ...
            results.append({"value": value, "original": value_str, "context": context})
    ...
    def generate_citations(self, answer):
        ...
        return {"cited_answer": cited_answer, "citations": citations}
        ...
        reasoning_chain = {
            ...
            "contradiction_count": len(
                [
                    ...
                    if any(
                        c.get("evidence1", "") == e_id or c.get("evidence2", "") == e_id
                        for e_id in [e for s in steps for e in s["evidence_ids"]]
                    )
                ]
            ),
        }
    ...
    def get_step_evidence(self, step_id: str) -> List[Dict]:
        ...
        for step in self.reasoning_steps:
            if step["step_id"] == step_id:
                ...
                for evidence_id in step["evidence_ids"]:
                    if evidence_id in self.evidence_items:
                        evidence_list.append(self.evidence_items[evidence_id])
    ...
    def summarize_reasoning(self, query_id: str) -> Dict:
        ...
        evidence_count = sum(len(step.get("evidence", [])) for step in chain.get("steps", []))

        # 识别关键步骤（有最多证据的步骤）
        key_steps = []
        if steps_count > 0:
            # 按证据数量排序
            sorted_steps = sorted(chain.get("steps", []), key=lambda x: len(x.get("evidence", [])), reverse=True)
