...
class ChainOfExplorationSearcher:
    ...
    def explore(self, query: str, starting_entities: List[str], max_steps: int = 5, exploration_width: int = 3):
        ...
        if not starting_entities:
            return {"entities": [], "relationships": [], "content": [], "exploration_path": []}
        ...
        for entity in starting_entities:
            self.exploration_path.append({"step": 0, "node_id": entity, "action": "start", "reasoning": "初始实体"})
        ...
        results = {"entities": [], "relationships": [], "content": [], "communities": []}
        ...
        for step in range(max_steps):
            ...
            # 2. 动态宽度控制
            current_width = self._calculate_adaptive_width(step, query, neighbors, base_width=exploration_width)

            # 3. 评估每个邻居与查询的相关性
            scored_neighbors = self._score_neighbors_enhanced(neighbors, query, query_embedding, exploration_strategy)
            ...
            for entity in new_entities:
                self.exploration_path.append(
                    {"step": step + 1, "node_id": entity, "action": "explore", "reasoning": reasoning}
                )
    ...
    def _estimate_query_complexity(self, query):
        ...
        # 识别复杂问题的关键词
        complexity_indicators = ["为什么", "如果", "原因", "关系", "比较", "区别", "影响", "分析", "评估", "预测"]
    ...
    def _get_neighbors(self, entities):
        ...
        try:
            ...
            params = {"entity_ids": entities, "visited_ids": list(self.visited_nodes)}
            ...
            try:
                ...
                if description:
                    ...
                    similarity = cosine_similarity(
                        np.array(query_embedding).reshape(1, -1), np.array(neighbor_embedding).reshape(1, -1)
                    )[0][0]
                else:
                    similarity = 0.0
    ...
    def _get_relationship_info(self, entities):
        ...
        try:
            ...
            result = self.graph.query(query, params={"entity_ids": entities, "visited_ids": list(self.visited_nodes)})
    ...
    def _rank_content_by_relevance(self, query_embedding, content_list):
        ...
        for content in content_list:
            ...
            try:
                ...
                # 计算相似度
                similarity = cosine_similarity(
                    np.array(query_embedding).reshape(1, -1), np.array(text_embedding).reshape(1, -1)
                )[0][0]
    ...
    async def explore_async(self, query: str, starting_entities: List[str], max_steps: int = 5):
        ...
        # 创建任务
        exploration_task = asyncio.create_task(self._explore_async_impl(query, starting_entities, max_steps))
