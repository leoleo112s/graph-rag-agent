...
class VectorUtils:
        ...
        for doc in docs:
            ...
            if doc_embedding:
                ...
                if similarity >= threshold:
                    scored_docs.append({"document": doc, "score": similarity})
            else:
                # 如果没有向量，给一个基础分数
                scored_docs.append({"document": doc, "score": 0.0})
    ...
    @staticmethod
    def batch_cosine_similarity(query_embedding: np.ndarray, embeddings: List[np.ndarray]) -> np.ndarray:
        """
        批量计算余弦相似度，提高效率
